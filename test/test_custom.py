...
class TestClass:
    def test_root(self):
        ...
        assert Root(use_namespace=True).render() == '<svg version="2.0" xmlns="http://www.w3.org/2000/svg" xmlns:xlink="http://www.w3.org/1999/xlink"></svg>'

    def test_xml_decl(self):
        assert XMLDeclaration().render() == '<?xml version="1.0" encoding="UTF-8"?>'
        assert XMLDeclaration(version="2.0").render() == '<?xml version="2.0" encoding="UTF-8"?>'

    def test_image(self):
        url = "https://example.com/example.jpg"

        assert Image(url).render() == f'<image href="{url}"/>'
        assert Image(url, use_xlink=True).render() == f'<image href="{url}" xlink:href="{url}"/>'
        assert Image(url, use_xlink_only=True).render() == f'<image xlink:href="{url}"/>'
